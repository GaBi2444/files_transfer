...
if conf.dataset == 'STAR':
    dataset_train = STAR(qa_path= conf.data_path+'/Question_Answer_SituationGraph/', split='train', mode="test",
...
#                       filter_small_box=False if conf.mode == 'predcls' else True)
    ...
    dataset_test = STAR(qa_path= conf.data_path+'/Question_Answer_SituationGraph/', split='val', mode="test", 
            datasize=conf.datasize, data_path=conf.data_path, filter_nonperson_box_frame=True,
            filter_small_box=False if conf.mode == 'predcls' else True)
...
#                      filter_small_box=False if conf.mode == 'predcls' else True)
    ...
    dataset_train = AG(mode="train", datasize=conf.datasize, data_path=conf.data_path, filter_nonperson_box_frame=True,
                                  filter_small_box=False if conf.mode == 'predcls' else True)
    dataloader_train = torch.utils.data.DataLoader(dataset_train, shuffle=True, num_workers=8,
                                                           collate_fn=cuda_collate_fn, pin_memory=False)
    dataset_test = AG(mode="test", datasize=conf.datasize, data_path=conf.data_path, filter_nonperson_box_frame=True,
                                 filter_small_box=False if conf.mode == 'predcls' else True)
    dataloader_test = torch.utils.data.DataLoader(dataset_test, shuffle=False, num_workers=8,
                                                collate_fn=cuda_collate_fn, pin_memory=False)
...
if conf.model == 'swin':
    swin = swin_init()
    object_detector = detector_swin(model = swin, train=True, object_classes=dataset_train.object_classes, use_SUPPLY=True, mode=conf.mode).to(device=gpu_device)
...
#tb
#writer1 = SummaryWriter(log_dir = conf.save_path + '/' + 'train_loss_00_' + conf.mode)
# some parameters
tr = []
resume = False
start_epoch = -1
...
for epoch in range(start_epoch + 1, conf.nepoch):
    ...
    for b in range(len(dataloader_train)):
        ...
        if b % print_num == 0 and b >= 0:
            ...
            print(mn)
            start = time.time()
            #writer1.add_scalar(tag = 'loss', scalar_value=loss, global_step=ite)
    ...
    torch.save(checkpoint_save, os.path.join(conf.save_path, "sttran_model_star_{}_{}.tar".format(conf.mode ,(epoch))))
    print("*" * 40)
