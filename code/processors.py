...
@registry.register_processor("vocab_unpadding")
class VocabUnpaddingProcessor(BaseProcessor):
    """Use VocabProcessor when you have vocab file and you want to process
    words to indices. Expects UNK token as "<unk>" and pads sentences using
    "<pad>" token. Config parameters can have ``preprocessor`` property which
    is used to preprocess the item passed and ``max_length`` property which
    ...
    **Key**: vocab
    ...
    def __call__(self, item):
        ...
        return {"text": indices, "tokens": tokens, "length": len(tokens)}
    ...
    def _map_strings_to_indices(self, tokens):
        #length = min(len(tokens), self.max_length)
        #tokens = tokens[:length]
        SPE_TOKEN = "pad,s,/s,unk,sep,mask,background,end,predict".split(',')

        output = torch.zeros(len(tokens), dtype=torch.long)
        #output.fill_(self.vocab.get_pad_index())
        for idx, token in enumerate(tokens):
            if token in SPE_TOKEN:
                token = '<'+token+'>'
            output[idx] = self.vocab.stoi[token]

        return output

@registry.register_processor("glove_unpadding")
class GloVeUnPaddingProcessor(VocabUnpaddingProcessor):
...
@registry.register_processor("token_glove")
class TokenGloVeProcessor(VocabProcessor):
    """Inherits VocabProcessor, and returns GloVe vectors for each of the
    words. Maps them to index using vocab processor, and then gets GloVe vectors
    corresponding to those indices.
    ...
    def __call__(self, words_seq):
        ...
        for words in words_seq:
            ...
            for w in words:
                temp_ = []
                for w_ in w:
                    index = self.vocab.stoi[w_]
                    embedding = self.vocab.vectors[index]
                    temp_.append(embedding)
                temp_ = torch.stack(temp_)
                temp_ = torch.mean(temp_, 0)
                temp.append(temp_)
            temp = torch.stack(temp)
            glove_embedding.append(temp)

        return torch.stack(glove_embedding) # [b, t, 300]
...
@registry.register_processor("graph_sampler")
class GraphSampler(BaseProcessor):
    """Label processor for multi class cases where the labels are
    saved in a file.
    """
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self.sample_type = config.type
        self.padding_key = 'padding_{}'
        self.unseen_ratio =  getattr(config, "unseen_ratio", 0.25)

    def uniform_sample(self,frame_ids, num):
        tick = len(frame_ids) / float(num)
        index = np.array([int(tick / 2.0 + tick * x) for x in range(num)])
        return [frame_ids[i] for i in index] 

    def balance_sample(self,frame_ids, unseen_index):

        seen_num = int(self.max_frame * (1 - self.unseen_ratio))
        unseen_num = self.max_frame - seen_num

        seen_part = frame_ids[:unseen_index]
        unseen_part = frame_ids[unseen_index:]
        sampled_id = []
        if len(seen_part) <= seen_num:
            sampled_id.extend(seen_part)
        else:
            seen_ids = self.uniform_sample(seen_part, seen_num)
            sampled_id.extend(seen_ids)

        if len(unseen_part) <= unseen_num:
            sampled_id.extend(unseen_part)
        else:
            unseen_ids = self.uniform_sample(unseen_part, unseen_num)
            sampled_id.extend(unseen_ids)

        return sampled_id


    def sampling(self, frame_ids, end_frame):

        frame_ids = list(sorted(frame_ids))

        if len(frame_ids)<=self.max_frame:
            return frame_ids

        if self.sample_type=='uniform':
            sampled_id = self.uniform_sample(frame_ids, self.max_frame)
            return sampled_id

        elif self.sample_type=='balance':
            
            unseen_index = -1
            for i, id_ in enumerate(frame_ids):
                if int(id_) > int(end_frame):
                    unseen_index = i
                    break
            ...
            if unseen_index == -1:
                ...
                sampled_id = self.balance_sample(frame_ids, unseen_index)
                return sampled_id

        else:
            return frame_ids[:self.max_frame]

    def __call__(self, frame_ids, end_frame):

        frame_dict = {}
        sampled_ids = self.sampling(frame_ids, end_frame)
        frame_attmask = []

        # 1: seen, 0: unseen
        for f in sampled_ids:
            if int(f) <= end_frame:
                frame_dict[f] = 1
                frame_attmask.append(1)
            else:
                frame_dict[f] = 0
                frame_attmask.append(1)

        if len(sampled_ids) < self.max_frame:
            pad_index = 0
            for i in range(self.max_frame - len(sampled_ids)):
                key = self.padding_key.format(str(pad_index))
                frame_dict[key] = 0
                pad_index += 1
                frame_attmask.append(0)

        return frame_dict, torch.tensor(frame_attmask, dtype=torch.long)
