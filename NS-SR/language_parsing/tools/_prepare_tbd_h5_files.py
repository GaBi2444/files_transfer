...
if args.q_type == 'oe':
    ...
    vocab_path = os.path.join(input_dir, 'mc_question_vocab.json')
else:
    vocab_path = os.path.join(input_dir, 'mc_choice_vocab.json')
...
with h5py.File(input_h5_path, 'r') as input_h5:
    ...
    image_idxs = np.asarray(input_h5['image_idxs'], dtype=np.int32)
...
with h5py.File(output_h5_path, 'w') as output_h5:
    ...
    output_h5.create_dataset('image_idxs', data=image_idxs)
