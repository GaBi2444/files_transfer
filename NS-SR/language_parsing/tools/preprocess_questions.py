...
parser.add_argument('--question_type', type=str, choices=['mc_question', 'mc_choice', 'oe_question'])
...
def main(args):
    print('loading questions', args.question_path)
    with open(args.question_path) as f:
        ann = json.load(f)
    ###########################
#     ann = ann[:30]
#     print('########, trimmed for testing, only 30 samples \n')
    
    
    # open_ended = ('answer' in ann[0]['questions'][0])
    all_qs, all_pgs, all_ans = [], [], []
    scene_idxs, q_idxs = [], []
    start_idx, end_idx = 0, len(ann)
    # if args.split == 'train':
    #     start_idx, end_idx = 0, int(end_idx*0.5)
    # elif args.split == 'val':
    #     start_idx, end_idx = int(end_idx*0.5), int(end_idx*0.75)
    # elif args.split == 'test':
    #     start_idx, end_idx = int(end_idx*0.75), end_idx
    ...
    for i in range(start_idx, end_idx):
        if True:
            ...
            if args.question_type == 'oe_question':
                all_qs.append(q['question'])
                all_pgs.append(tmp_split.join(q['program']))
                all_ans.append(q['answer'])
                scene_idxs.append(i)
                q_idxs.append(j)

            elif args.question_type == 'mc_question':

                for choice in q['choices']:
                    all_qs.append(q['question'].strip('?').strip('.')+' '+choice['choice'].strip('.'))
                    all_pgs.append(tmp_split.join(q['program']+choice['op_pro']))
                    scene_idxs.append(i)
                    q_idxs.append(int(q['question_id'].split('_')[-1]))

            elif args.question_type == 'mc_choice':
                chs, ch_pgs, ans = process_choices(q)
                all_qs += chs
                all_pgs += ch_pgs
                all_ans += ans
                scene_idxs += [i for ic in range(len(chs))]
                q_idxs += [j for ic in range(len(chs))]
            else:
                raise ValueError('Invalid question type')

    
    # Stemming Processing
    if args.question_type == 'oe_question' or args.question_type == 'mc_question':
        all_qs = [' '.join( word_stemmer(qs.split()) ) for qs in all_qs]
        all_pgs = pd.DataFrame(all_pgs)
        all_pgs = all_pgs[0].str.replace(' .*?,',tmp_split,regex=True).str.split(tmp_split).values
        all_ans = [' '.join( word_stemmer(ans.split()) ) for ans in all_ans]
    elif args.question_type == 'mc_choice':
        print( 'TODO: mc_choice mode is used for parsing choices')
    ...
    vocab = json.load(open('./output_data/rewrite/STAR_human_train_vocab.json'))
...
#         print('q_tokens',q_tokens)
        ...
        if args.question_type != 'mc_question':
            an_encoded = vocab['answer_token_to_idx'][all_ans[i]]
            ans_encoded.append(an_encoded)
    ...
    ans_encoded = np.asarray(ans_encoded, dtype=np.int32)
    print('qs_encoded.shape',qs_encoded.shape)
    print('pgs_encoded.shape',pgs_encoded.shape)
    print('ans_encoded.shape',ans_encoded.shape)
    with h5py.File(args.output_h5_path, 'w') as fout:
        ...
        fout.create_dataset('answers', data=ans_encoded)
        fout.create_dataset('video_idxs', data=np.asarray(scene_idxs))
