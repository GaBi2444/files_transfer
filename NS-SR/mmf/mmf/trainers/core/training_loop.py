...
class TrainerTrainingLoopMixin(ABC):
    ...
    def run_training_epoch(self) -> None:
        should_break = False
        writer = SummaryWriter(self.training_config.vis_dir)
        ...
        while self.num_updates < self.max_updates and not should_break:
            ...
            for idx, batch in enumerate(self.train_loader):
                ...
                if self.training_config.visualize:
                    ...
                    item1 = batch.item1
                    item2 = batch.item2
                    whole_frames = batch.whole_frames
                    visualize_images(writer,human_pose,'human_pose',idx)
                    visualize_images(writer,item1,'item1',idx)
                    visualize_images(writer,item2,'item2',idx)
                    visualize_images(writer,whole_frames,'whole_frames',idx)
                ...
                if self.num_updates % self.training_config.evaluation_interval == 0:
                    ...
                    # Create a new meter for this case
                    report, meter = self.evaluation_loop(self.val_loader,use_tqdm=True)
