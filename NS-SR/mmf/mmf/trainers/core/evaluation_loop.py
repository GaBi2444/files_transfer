        ...
        with torch.no_grad():
            ...
            for batch in tqdm.tqdm(loader, disable=disable_tqdm):
                ...
                if combined_report is None:
                    ...
                    metrics = self.metrics(report,report)
                    if accmulate_metrics is None:
                        accmulate_metrics = metrics
                    else:
                        for item in metrics:
                            accmulate_metrics[item] += metrics[item]
                    #print(accmulate_metrics)
                else:
                    batch_size += report.batch_size
                    metrics = self.metrics(report,report)
                    if accmulate_metrics is None:
            ...
            for item in accmulate_metrics:
                accmulate_metrics[item] = accmulate_metrics[item][0] / accmulate_metrics[item][1]
            #print('final', accmulate_metrics)
            combined_report.batch_size = batch_size
            combined_report.metrics = accmulate_metrics
            self.update_meter(combined_report, meter, eval_mode=True)
            # enable train mode again
            self.model.train()
