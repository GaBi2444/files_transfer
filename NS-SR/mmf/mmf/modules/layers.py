...
class PoseConvNet(nn.Module):
    def __init__(self,in_channels=3):
        super().__init__()

        self.conv1 = ConvNet(in_channels,32,3)
        self.conv2 = ConvNet(32,64,3)
        self.conv3 = ConvNet(64,128,3)
        self.conv4 = ConvNet(128,256,3)
...
class VideoEncoderCNN(nn.Module):
    def __init__(self, feature_dim,mode):
    ...
    def forward(self, images_squence):
        """Extract feature vectors from input images."""
        if self.mode == 'avg':
            with torch.no_grad():
                features = torch.stack([self.resnet(image).reshape(image.size(0), -1) for image in images_squence])
            features = self.bn(self.linear(torch.mean(features,dim=1)))
            return features

        elif self.mode == 'concate':
            with torch.no_grad():
                #features = torch.stack([torch.cat([feat for feat in self.resnet(image).reshape(image.size(0), -1)],dim=-1)  for image in images_squence])
                features = torch.stack([self.resnet(image).reshape(image.size(0), -1) for image in images_squence])
            features = torch.stack([torch.cat([feat for feat in self.bn(self.linear(feature))],dim=-1)  for feature in features])
        #print("after_bn",features.shape)
            return features