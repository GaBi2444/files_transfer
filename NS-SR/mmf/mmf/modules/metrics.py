...
class Accuracy(BaseMetric):
    ...
    **Key:** ``accuracy``
    ...
    def __init__(self):
        ...
        self.required_params = ["scores", "targets",'question_id']
...
def evaluate(output,expected):
    ...
    try:    
        value = (correct / total).tolist()
    except:
        value = 0
    return value


def group_by_qtype(question_ids,output,expected):
    interaction = {'output':[],'target':[]}
    sequence =  {'output':[],'target':[]}
    prediction =  {'output':[],'target':[]}
    feasibility =  {'output':[],'target':[]}

    for i, question_id in enumerate(question_ids):
        #question_id = qa['question_id']
        q_type = question_id.split('_')[0]
        out = output[i]
        target = expected[i]

        if q_type == 'Feasibility':
            #feasibility['id'].append(question_id)
            feasibility['output'].append(out.cpu().tolist())
            feasibility['target'].append(target.cpu().tolist())
        if q_type == 'Prediction':
            #prediction['id'].append(question_id)
            prediction['output'].append(out.cpu().tolist())
            prediction['target'].append(target.cpu().tolist())

        if  q_type == 'Sequence':
            #sequence['id'].append(question_id)
            sequence['output'].append(out.cpu().tolist())
            sequence['target'].append(target.cpu().tolist())

        if  q_type == 'Interaction':
            #interaction['id'].append(question_id)
            interaction['output'].append(out.cpu().tolist())
            interaction['target'].append(target.cpu().tolist())

    return interaction,sequence,prediction,feasibility

@registry.register_metric("per_question_accuracy")
class PQAccuracy(BaseMetric):
    """Metric for calculating accuracy.
    ...
    **Key:** ``accuracy``
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        output = model_output["scores"]
        expected = sample_list["targets"]
        question_id = sample_list["question_id"]

        interaction,sequence,prediction,feasibility = group_by_qtype(question_id,output,expected)

        if len(interaction['output'])!=0:
            int_acc = evaluate(torch.tensor(interaction['output']),torch.tensor(interaction['target']))
        else:
            int_acc = 0
        if len(sequence['output'])!=0:
            seq_acc = evaluate(torch.tensor(sequence['output']),torch.tensor(sequence['target']))
        else:
            seq_acc = 0
        if len(prediction['output'])!=0:
            pre_acc = evaluate(torch.tensor(prediction['output']),torch.tensor(prediction['target']))
        else:
            pre_acc = 0
        if len(feasibility['output'])!=0:
            fea_acc = evaluate(torch.tensor(feasibility['output']),torch.tensor(feasibility['target']))
        else:
            fea_acc = 0

        print('interaction per question: ', int_acc)
        print('sequence per question: ',seq_acc)
        print('prediction per question: ', pre_acc)
        print('feasibility per question: ', fea_acc)

        count, mean_acc = 0, 0
        for acc in [int_acc,seq_acc,pre_acc,fea_acc]:
            if acc != 0:
                mean_acc+=acc
                count +=1
...
@registry.register_metric("mask_act_acc_bkp")
class MaskActAcc(BaseMetric):
    """Metric for calculating accuracy.
    ...
    **Key:** ``accuracy``
    ...
    def __init__(self):
        super().__init__("mask_act_acc")

        self.required_params = ["pre_act_cls", "act_target"]
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        """
        output = model_output["pre_act_cls"]
        expected = sample_list["act_target"]
        #print('expected',expected.shape) # 518, 16, 2
        #print('output',output.shape) 
        b, _, _ = expected.shape
        expected = expected.reshape(b,-1)
        output = output.reshape(b,-1)

        mask = expected!=0
        # action_mask = (expected>=self.ACT_S) * (expected<self.ACT_E)
        # obj_mask = (expected>=self.ACT_E) * (expected<self.REL_E)
        # rel_mak = (expected>=self.REL_E) 

        output_ = output[mask]
        expected_ = expected[mask]
        correct = (output_ == expected_).sum().float()
        total = mask.sum()
        value = correct / total

        return value

@registry.register_metric("mask_act_acc")
class MaskActAcc(BaseMetric):
    """Metric for calculating accuracy.
...
@registry.register_metric("mask_obj1_acc")
class MaskObj1Acc(BaseMetric):
    """Metric for calculating accuracy.
    ...
    **Key:** ``accuracy``
    ...
    def __init__(self):
        super().__init__("mask_obj1_acc")

        self.required_params = ["pre_obj1_cls", "obj1_target"]
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        """
        output = model_output["pre_obj1_cls"]
        expected = sample_list["obj1_target"]
        #print('expected',expected.shape) # 518, 16, 2
        #print('output',output.shape) 
        b, _, _ = expected.shape
        expected = expected.reshape(b,-1)
...
@registry.register_metric("mask_rel_acc")
class MaskRelAcc(BaseMetric):
    """Metric for calculating accuracy.
    ...
    **Key:** ``accuracy``
    ...
    def __init__(self):
        super().__init__("mask_rel_acc")

        self.required_params = ["pre_rel_cls", "rel_target"]
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        """
        output = model_output["pre_rel_cls"]
        expected = sample_list["rel_target"]
        #print('expected',expected.shape) # 518, 16, 2
        #print('output',output.shape) 
        b, _, _ = expected.shape
        expected = expected.reshape(b,-1)
...
@registry.register_metric("mask_obj2_acc")
class MaskObj2Acc(BaseMetric):
    """Metric for calculating accuracy.
    ...
    **Key:** ``accuracy``
    ...
    def __init__(self):
        super().__init__("mask_obj2_acc")

        self.required_params = ["pre_obj2_cls", "obj2_target"]
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        """
        output = model_output["pre_obj2_cls"]
        expected = sample_list["obj2_target"]
        #print('expected',expected.shape) # 518, 16, 2
        #print('output',output.shape) 
        b, _, _ = expected.shape
        expected = expected.reshape(b,-1)
        ...
        return value
@registry.register_metric("act_pre_acc")
class ActionAccuracy(BaseMetric):
    """Metric for calculating accuracy.
    ...
    **Key:** ``accuracy``
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        """
        output = model_output["predict_situation"]
        #output_action = 
        expected = sample_list["mask_lm_label"]
