...
class TestReporter(Dataset):
    ...
    def flush_report(self):
        ...
        qtype = getattr(self.dataset_config.star_nssr,'qtype', None)
        ...
        filename = filename + '_' + self.experiment_name

        #filename += self.task_type
        #filename += time
    ...
    def json_dump_a(self, filepath):
        with PathManager.open(filepath, "a+") as f:
            json.dump(self.report, f)
    ...
    def add_to_report(self, report, model):
        ...
        #keys = ['act_index','obj1_index','rel_index','obj2_index']
        keys = []
