...
class BaseVocab:
    ...
    def __init__(
        ...
        self.itos[self.SEP_INDEX] = self.SEP_TOKEN
        self.itos[self.MASK_INDEX] = self.MASK_TOKEN

        self.word_dict[self.PAD_TOKEN] = self.PAD_INDEX
        self.word_dict[self.UNK_TOKEN] = self.UNK_INDEX
        self.word_dict[self.SEP_TOKEN] = self.SEP_INDEX
        self.word_dict[self.MASK_TOKEN] = self.MASK_INDEX
...
class CustomVocab(BaseVocab):
    def __init__(self, vocab_file, embedding_file, data_dir=None, *args, **kwargs):
        ...
        for i in range(9, self.get_size()):
            self.vectors[i] = embedding_vectors[i - 9]
