...
class TestMeter(object):
    ...
    def __init__(
        ...
        self.total_clips = num_videos * cfg.TEST.NUM_ENSEMBLE_VIEWS
        ...
        self.topk_accs = []
        self.stats = {}
        self.clips_preds = torch.zeros((self.total_clips, num_cls))
        self.clips_vid = []
        self.clips_fid = []
        # Reset metric.
        self.reset()
    
    def update_clips_status(self, preds, vid_fid, clip_ids):
        for ind in range(preds.shape[0]):
            clip_id = int(clip_ids[ind]) // self.num_spatial_crops
           
            self.clips_vid.append(vid_fid['vid'][ind])
            clip_frames = []
            for i in range(len(vid_fid['fid'])):
                clip_frames.append(vid_fid['fid'][i][ind])
            self.clips_fid.append(clip_frames)
            if self.ensemble_method == "sum":
                self.clips_preds[clip_id] += preds[ind]
            elif self.ensemble_method == "max":
                self.clips_preds[clip_id] = torch.max(
                    self.clips_preds[clip_id], preds[ind]
                )
            else:
        ...
        #embed()
    def final_clips_metrics(self, top_k = 5):
        #print(self.clips_preds[1:10])
        for i in range(self.clips_preds.shape[0]):
            self.clips_preds[i] = self.clips_preds[i] / sum(self.clips_preds[i])
        scores_pred_label, pred_label = torch.topk(self.clips_preds, top_k)
        scores_all, pred_label = torch.topk(self.clips_preds, cfg.MODEL.NUM_CLASSES)
        embed()
        video_frame_preds = {}

        #pred_label = pred_label.numpy().tolist()
        #clips_vid = self.clips_vid.numpy().tolist()
        #clips_fid = self.clips_fid.numpy().tolist()   
        frames_query = {}
        for i in range(len(self.clips_vid)):
            if i % self.num_spatial_crops == 0 :
                vid = self.clips_vid[i]
                fids = self.clips_fid[i]
                label = pred_label[i//3].numpy().tolist()
                frames = {}
                if vid not in video_frame_preds:
                    for fid in fids:
                        if fid not in frames:
                            frames[fid] = label
                    video_frame_preds[vid] = frames
                else:
                    for fid in fids:
                        frames_exist = video_frame_preds[vid]
                        if fid not in frames_exist:
                            video_frame_preds[vid][fid] = label
                        else:
                            labels_exist = frames_exist[fid]
                            for l in label:
                                if l not in labels_exist:
                                    video_frame_preds[vid][fid].append(l)
                                else:
                                    continue
        #embed()
